...
- **Top-level `version`** (string): tracks schema + content revisions. Bumped whenever the file changes meaningfully. Currently `"3.3.1"`. Must move in lockstep with `cc-switcher.psd1`'s `ModuleVersion`, `cc-switcher.psm1`'s `$script:CCSwitcherVersion`, and `bash/cc-switcher.sh`'s `CCSWITCHER_VERSION`.
